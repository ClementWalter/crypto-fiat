...
function updateMood(event) {
  ...
  chrome.storage.sync.set({ moodValue }, () => {
    ...
    facebookTabIds.map((tabId) =>
      chrome.tabs.sendMessage(tabId, { moodValue })
    );
  });
}
...
function updateSpecialLiker(event) {
  ...
  chrome.storage.sync.set({ specialLiker }, () => {
    console.log(`Special Liker set to ${specialLiker}`);
    facebookTabIds.map((tabId) =>
      chrome.tabs.sendMessage(tabId, { specialLiker })
    );
  });
}
...
function updateAverageUplikeInterval(event) {
  ...
  chrome.storage.sync.set({ averageUpLikeInterval }, () => {
    console.log(`Average uplike interval set to ${averageUpLikeInterval}`);
    facebookTabIds.map((tabId) =>
      chrome.tabs.sendMessage(tabId, { averageUpLikeInterval })
    );
  });
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelector('input[name="special-liker"]').onchange =
    updateSpecialLiker;
  document.querySelector('input[name="average-uplike-interval"]').onchange =
    updateAverageUplikeInterval;
  ...
  chrome.storage.sync.get(
    ...
    (result) => {
      ...
      document.querySelector('input[name="special-liker"]').value =
        specialLiker;
      document.querySelector('input[name="average-uplike-interval"]').value =
        averageUpLikeInterval;
